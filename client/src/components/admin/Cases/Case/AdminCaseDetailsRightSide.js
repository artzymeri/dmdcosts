    ...
    return (
        <div className="admin-case-details-right-side">
          ...
          <span className="admin-case-details-right-side-qr-code-container-description-title">
            ...
            <Button size="large" fullWidth variant="contained" color={presenter.caseDetails?.paid ? 'error' : 'success'}
                    onClick={() => {
                        presenter.handleCasePaymentChange(!presenter.caseDetails.paid).then((res) => {
                            presenter.setSnackbar(true, res.data);
                        })
                    }}>
